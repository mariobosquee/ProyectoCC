...
def generar_grafica_apilada(request):
    logger.info("Petición a generar_grafica_apilada: %s", request.POST.dict())
...
def generar_grafica_circular(request):
    logger.info("Petición a generar_grafica_circular: %s", request.POST.dict())
    # Obtiene parámetros del POST
    ...
    if anio_fin < anio_inicio:
        logger.warning("Año de fin < Año de inicio: anio_inicio=%s, anio_fin=%s", anio_inicio, anio_fin)
    ...
    if not nacionalidades or not anio_inicio or not anio_fin or isinstance(anio_inicio, int) or isinstance(anio_fin, int):
        logger.warning("Entrada inválida: nacionalidades=%s, anio_inicio=%s, anio_fin=%s", nacionalidades, anio_inicio, anio_fin)
...
def generar_grafica_lineas(request):
    logger.info("Petición a generar_grafica_lineas: %s", request.POST.dict())
    # Obtiene parámetros del POST
    anio = request.POST.get('anio')
    solo_mortales = request.POST.get('solo_mortales') == 'true'
...
def generar_diagrama_dispersion(request):
    logger.info("Petición a generar_diagrama_dispersion: %s", request.POST.dict())
    # Obtiene parámetros del POST
...
def generar_mapa(request):
    logger.info("Petición a generar_mapa: %s", request.POST.dict())
    # Obtiene parámetros del POST
    solo_mortales = request.POST.get('solo_mortales', 'false') == 'true'
    ...
    try:
        ...
        # Verifica que hay datos antes de hacer merge para evitar error KeyError
...
def generar_histograma(request):
    logger.info("Petición a generar_histograma: %s", request.POST.dict())
    # Obtiene parámetros del POST
    solo_mortales = request.POST.get('solo_mortales', 'false') == 'true'
...
def generar_radar(request):
    logger.info("Petición a generar_radar: %s", request.POST.dict())
    # Obtiene parámetros del POST
...
def comparativa_sklearn(request):
    logger.info("Petición a comparativa_sklearn: %s", request.POST.dict())
    # Obtiene parámetros del POST
    ...
    if (not actividad and not localizacion and not zonavigilada and not factorriesgo and not intervencion and not edad):
        logger.warning("Falta filtro o edad: actividad=%s, localizacion=%s, zonavigilada=%s, factorriesgo=%s, intervencion=%s, edad=%s",
...
def generar_mapa_hotspots(request):
    logger.info("Petición a generar_mapa_hotspots: %s", request.POST.dict())
    # Obtiene parámetros del POST
...
def generar_kmeans(request):
    logger.info("Petición a generar_kmeans: %s", request.POST.dict())
    # Obtiene parámetros del POST
    ...
    try:
        ...
        logger.info(f"Gráfico K-Means generado para período {anio_inicio}-{anio_fin} con {k} clusters.")
...
def generar_tree(request):
    logger.info("Petición a generar_tree: %s", request.body.decode('utf-8'))
